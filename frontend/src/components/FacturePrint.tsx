...
import { UIFacture } from '../services/api';

interface FacturePrintProps {
  facture: UIFacture;
}

export const FacturePrint: React.FC<FacturePrintProps> = ({ facture }) => {
  const taxRate = facture.taxRate ?? 19;
  const fiscalStamp = facture.fiscalStamp ?? 1.0;
  const totalHT = (facture.items || []).reduce((sum, item) => sum + (item.quantity * item.unitPrice), 0);
  ...
  const content = (
    <div className="facture-print-container">
      <style>{`
        @media screen {
          .facture-print-container { display: none !important; }
        }
            ...
            <td>
              <div className="doc-summary">
                <div className="doc-identity">
                  ...
                  <p>Date d'émission: <strong>{facture.issuedAt}</strong></p>
                  {facture.dueAt && <p>Echéance: <strong>{facture.dueAt}</strong></p>}
                </div>
                <div className="recipient-box">
                  <span className="recipient-label">FACTURE À / DESTINATAIRE</span>
                  <p className="recipient-name">{facture.clientName}</p>
                </div>
              </div>
                ...
                <tbody>
                  {(facture.items || []).map((item, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="align-right">{(item.quantity * item.unitPrice).toLocaleString('fr-FR')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="summary-section">
            ...
            <div className="summary-card">
              <div className="summary-row">
                ...
                <span style={{ fontWeight: 800 }}>{totalHT.toLocaleString('fr-FR')} {facture.devise}</span>
              </div>
              <div className="summary-row">
                <span style={{ color: '#6b7280' }}>TVA ({taxRate}%)</span>
                <span style={{ fontWeight: 800 }}>{tvaAmount.toLocaleString('fr-FR')} {facture.devise}</span>
              </div>
              <div className="summary-row">
                <span>TOTAL TTC</span>
                <span>{totalTTC.toLocaleString('fr-FR')} {facture.devise}</span>
              </div>
            </div>
          </div>
        </div>
